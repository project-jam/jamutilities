...
const getUserBadges = (user: User, member?: GuildMember) => {
    ...
    if (member?.premiumSince) {
        const days = Math.floor(
            (Date.now() - member.premiumSince.getTime()) /
                (1000 * 60 * 60 * 24),
        );
...
export const command: Command = {
    data: new SlashCommandBuilder()
        ...
        .addUserOption((opt) =>
            opt
                .setName("user")
                .setDescription("User (leave empty for self)")
                .setRequired(false),
        )
        ...
        .addBooleanOption((opt) =>
            opt
                .setName("usrbg")
                .setDescription("Show generated background when no banner")
                .setRequired(false),
        ),

    prefix: {
        aliases: ["userinfo", "whois", "user"],
        usage: "[@user] [server:false] [usrbg:false]",
    },
    ...
    async execute(
        ...
        try {
            ...
            if (isPrefix) {
                ...
                if (lc.includes("usrbg:false") || lc.includes("usrbg=0"))
                    showUsrbg = false;
            } else {
            ...
            console.log(
                `User: ${target.tag}, Banner URL: ${discordBanner}, Show Usrbg: ${showUsrbg}`,
            );
            ...
            if (discordBanner) {
                ...
                const usrbgUrl = `https://usrbg.is-hardly.online/usrbg/v2/${target.id}`;
                try {
            ...
            if (imageUrl) {
                ...
                const key = discordBanner
                    ? "User Banner"
                    : "Generated Background";
                embed.addFields({
                    name: `🎨 ${key}`,
                    value: `[Click to view](${imageUrl})`,
                });
            }
            ...
            if (isPrefix)
                await (interaction as Message).reply({ embeds: [ebd] });
            else
                await (interaction as ChatInputCommandInteraction).editReply({
                    embeds: [ebd],
                });
